...
<div class="row mt-4 mb-3">
 ...
 <div class="container shadow p-4">
      <form method="POST" action="{{url('admin/news_update/'.$data->id)}}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="col-sm-2 my-2">
                  ...
                  <select class="form-select" name="category_name_id" id="category_name_id" aria-label="Default select example" required>
                      <option   value=""> Select One </option>
                        @foreach(category() as $row)
                            @if($row->category_name==($data?$data->category_name_id:""))
                                <option  value="{{$row->category_name}}" selected> 
                                 {{$row->name}}</option>
                            @else
          ...
          <div class="col-sm-2 my-2">
                      ...
                      <select class="form-select" name="division_id" id="division_id" aria-label="Default select example" >
                        <option   value=""> Select One </option>
                          @foreach(division() as $row)
                             @if($row->id==($data?$data->division_id:""))
           ...
           <div class="col-sm-9 my-2">
              ...
              <textarea name="desc" id="summernote1" cols="30" rows="10" > {{ $data->desc }} </textarea required>
          </div>
          ...
          <div class="col-sm-3 my-2">
              ...
              <input type="text" name="image_title1" id="image_title1" value="{{ $data->image_title1 }}" class="form-control" > 
           </div>

           <div class="col-sm-9 my-2">
              <label for="Textarea" class="form-label"> Description 1</label>
              <textarea name="desc1" id="summernote2" cols="30" rows="10" > {{ $data->desc1 }}  </textarea >
          </div>
